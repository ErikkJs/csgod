...
{ 
    ...
    g.nobjects++;
  //------------------------------------------------------------------------
//-------------------Make the Cube faces and vertices--------------------------------------------
		int f[][3] = {
			{0,1,2}, {0,2,3}, {4,7,6},
			{4,6,5}, {0,3,7}, {0,7,4},
			{7,3,2}, {7,2,6}, {1,5,6},
...
//-------------------construct with triagnles -----------------------------------
		for (int i=0; i<12; i++) {
			o = &g.object[g.nobjects];
			o->type = TYPE_TRIANGLE;
			vecMake(v[f[i][0]][0] * 70,v[f[i][0]][1] * 70,v[f[i][0]][2] * 70,o->tri[0]);
			vecMake(v[f[i][1]][0] * 70,v[f[i][1]][1] * 70,v[f[i][1]][2] * 70,o->tri[1]);
			vecMake(v[f[i][2]][0] * 70,v[f[i][2]][1] * 70,v[f[i][2]][2] * 70,o->tri[2]);
			vecMake(0.9,0.5,0.2,o->color);
			o->clear_clips();
			vecMake(70, 70.0, 70.0, o->clip[o->nclips].center);
    ...
    o = &g.object[g.nobjects];
		o->type = TYPE_SPHERE;
		vecMake(25.0, 25.0, 25.0, o->center);
		vecMake(.9,.9,.2, o->color);
		o->radius = 30.0;
		o->surface = SURF_NONE;
		o->clear_clips();

		vecMake(0*70,1*70,0*70, o->clip[o->nclips].center);
		vecMake(0,1,0, o->clip[o->nclips].normal);
		vecNormalize(o->clip[o->nclips].normal);
		o->clip[o->nclips].radius = 50.0;
		o->clip[o->nclips].inside = true;
		++o->nclips;

		vecMake(1*70,0*70,0*70, o->clip[o->nclips].center);
		vecMake(1,0,0, o->clip[o->nclips].normal);
		vecNormalize(o->clip[o->nclips].normal);
		o->clip[o->nclips].radius = 50.0;
		o->clip[o->nclips].inside = true;
		++o->nclips;
		...
		vecMake(0*25,0*25,1*25, o->clip[o->nclips].center);
		vecMake(0,0,1, o->clip[o->nclips].normal);
		vecNormalize(o->clip[o->nclips].normal);
		o->clip[o->nclips].radius = 50.0;
		o->clip[o->nclips].inside = true;
		++o->nclips;
...
vecMake(90.0, 1500.0, 5000.0, g.lightPos);
	vecMake(350.0, 300.0, 900.0, g.from);
...
void init_disks(void)
{
    ...
    for(int i=0; i<10; i++){
        o = &g.object[g.nobjects];
        o->type = TYPE_DISK;
        vecMake(0.0, y , 0.0, o->center);
        vecMake(0.0, -1.0, 0.0, o->norm);
        // set radius
        o->radius= 150;
        o->specular = true;
        vecMake(0.2, 0.2, 0.2, o->spec);
        vecMake(.8, z , 0.0, o->color);
        o->surface =  SURF_NONE;
        vecNormalize(o->norm);

        //clipping the disks 
        vecMake(5000000.0, 0.0, 0.0, o->clip[o->nclips].center);
        o->clip[o->nclips].radius=5000000.0;
        ++o->nclips;
        //sphere in the middle
        vecMake(0.0, y , a , o->clip[o->nclips].center);
        o->clip[o->nclips].radius=65.0;
        ++o->nclips;
    ...
    //setup light and camera
    vecMake(-70.0, 80.0, 0.0, g.lightPos);
    vecMake(505.0, 510.0, 900.0, g.from);
    vecMake(0.0, 80.0, 0.0, g.at);
    vecMake(0.0, 1.0, 0.0, g.up);
    g.angle = 30.0;
}
...
void init_sphere(void)
{
    ...
    vecMake(.3,.7,.2, o->color);
    o->radius = 100.0;
    o->surface = SURF_NONE;
    g.nobjects++;

    o->clip[o->nclips].radius =100;
    vecMake(40.0, 170.0, 20.0, o->clip[o->nclips].center);
    o->clip[o->nclips].inside =true;
    ++o->nclips;

    //--------------------------------------------------------------------
    //sphere clipping big one
    o = &g.object[g.nobjects];
    o->type = TYPE_SPHERE;
    vecMake(40.0, 170.0, 20.0, o->center);
    o->specular = true;
    vecMake(0.5, 0.5, 0.5, o->spec);
...
void init(void)
{
    ...
    o->clip[o->nclips].radius =100;
    vecMake(40.0, 70.0, 20.0, o->clip[o->nclips].center);
...
//---------------------third part-------------------------------------------		
    ...
    //setup light and camera
    vecMake(90.0, 150.0, 500.0, g.lightPos);
    vecMake(304.0, 300.0, 1100.0, g.from);
    vecMake(0.0, 80.0, 0.0, g.at);
    vecMake(0.0, 1.0, 0.0, g.up);
    g.angle = 30.0;
}
...
void showMenu()
{
    ...
    x11.drawText(20, y, "s - Render sphere");
    y += inc;
    x11.setColor3i(255, 255, 0);
    x11.drawText(20, y, " 1- Render cube");
    y += inc;
     x11.setColor3i(255, 255, 0);
    x11.drawText(20, y, " 2- Render rings");
    y += inc;
     x11.setColor3i(255, 255, 0);
    x11.drawText(20, y, " 3- Render Pokeball");
    y += inc;
    x11.setColor3i(255, 255, 0);
    x11.drawText(20, y, "Q - RenderRings");
    y += inc;
}
