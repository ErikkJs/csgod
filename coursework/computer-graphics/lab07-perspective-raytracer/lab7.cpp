...
void init(void)
{
	...
	g.nobjects++;
	//--------------------------------------------------------------------
	//sphere sitting on tip
	o = &g.object[g.nobjects];
	o->type = TYPE_SPHERE;
	vecMake(0.0, 150.0, 0.0, o->center);
	vecMake(0.5, 0.5, 0.5, o->spec);
	vecMake(0,.3,.9, o->color);
	o->radius = 41;
	o->surface = SURF_NONE;
	g.nobjects++;
	//--------------------------------------------------------------------
   ...
   //ring
	...
	o->type = TYPE_DISK;
	o->inside = true;
	vecMake(0.0, 22.0, 0.0, o->center);
	vecMake(0.0, 10.0, 0.0, o->norm);
	vecMake(0,.3,.9, o->color);
	o->radius = 90.0;
	o->surface = SURF_NONE;
	g.nobjects++;
	//--------------------------------------------------------------------
	//cylinder sitting inside cylinder
	o = &g.object[g.nobjects];
	o->type = TYPE_CYLINDER;
	vecMake(0.0, 0.0, 0.0, o->center);
	vecMake(0.0, 0.0, 1.0, o->norm);
   ...
   g.nobjects++;
	...
	o = &g.object[g.nobjects];
	o->type = TYPE_RING;
	vecMake(0.0, 36.0, 0.0, o->center);
	vecMake(0.0, 8.0, 0.0, o->norm);
	vecMake(0,.3,.9, o->color);
	o->radius = 90.0;
   o->radius2= 95;
	o->surface = SURF_NONE;
	...
	vecMake(110.0, 300.0, 500.0, g.lightPos);
	vecMake(4.0, 500.0, 900.0, g.from);
