	...
	Game() {
		...
		for (int j=0; j<10; j++) {
			...
			cout << randomcolor << endl;	
		        float color = (Flt)randomcolor/255;	
			a->color[0] = color;
			a->color[1] = color;
			a->color[2] = color;
		...
		if (e->xbutton.button==1) {
			...
			if (ts > 0.1) {
				...
				if (g.nbullets < MAX_BULLETS) {
					...
					Flt rad = ((g.ship.angle+90.0) / boundary) * PI * 2.0;
	...
	if (e->type == MotionNotify) {
		if (savex != e->xbutton.x || savey != e->xbutton.y) {
			...
			if (xdiff > minimum) {
				...
				if (g.ship.angle >= boundary)
					g.ship.angle -= boundary;
			}
			else if (xdiff < minimum) {
				g.ship.angle += 0.05f * (float)xdiff;
				if (g.ship.angle < minimum)
					g.ship.angle += boundary;
			}
			if (ydiff > minimum) {
